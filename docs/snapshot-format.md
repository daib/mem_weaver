...
- **Written**: once per `SNAPSHOT_INTERVAL_SECS` check, subject to two gates: (1) the collection must have at least `SNAPSHOT_MIN_DIRTY_VECTORS` new vectors across all its buckets since the last snapshot; (2) individual buckets are only uploaded if they have dirty blocks. If the collection is below the threshold, the cycle is skipped entirely — no uploads, no `wal_high_seq` advancement. Setting `SNAPSHOT_MIN_DIRTY_VECTORS=0` (default) disables the threshold and snapshots on every cycle whenever any bucket is dirty.

  **Recommended threshold:** WAL replay is cheap — a few thousand vectors takes milliseconds to re-insert. Snapshotting arena files is expensive — each 2 MB block is an S3 PUT. A threshold around `10_000` is a sensible starting point: anything below that is fast to replay and not worth paying the upload cost for. Higher-throughput workloads can use a larger threshold (e.g. `100_000`) to batch more work per snapshot and reduce S3 write amplification. The right value is `throughput × acceptable_replay_time_on_crash`.
