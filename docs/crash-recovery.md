...
───────────────────────────────────────────────────────────────────────
  ...
  4. Ack client  ←  only after blob storage confirms
...
3. **Replay WAL** — list all WAL entries at `<collection>/wal/`. Filter to `seq > wal_high_seq` (from step 2). Sort ascending. For each entry, re-insert all vectors into the restored index.
...
|---|---|
| Before WAL local write | Insert was never applied; client never acked. No loss. |
| After local write, before blob upload | Local file survives. Uploaded on next startup. |
...
After a snapshot cycle in which every bucket succeeds, WAL entries with `seq <= wal_high_seq` are deleted from blob storage. If any bucket fails, `wal_high_seq` is not advanced and no WAL entries are pruned that cycle — they are kept until the next full cycle succeeds. This prevents data loss from partial snapshots. Local WAL files are removed immediately after each successful upload to blob storage.
